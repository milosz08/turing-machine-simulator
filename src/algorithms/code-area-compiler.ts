...
import { CompileAndRun, TapeValues, TupleDefinitions } from "~/app-utils/machine-config";
...
class CodeAreaCompiler {
    ...
    public setInitialInput(): string {
        ...
        if (getInitialInput) {
            const [ start, end ] = getInitialInput.split("").map((c, i) => c === $ ? i : []).filter(v => Number(v) > 0);
            if (!start || !end) {
                return "";
            } else {
