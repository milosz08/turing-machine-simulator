 ...
 *
 *   File name: code-area-compiler.ts
 *   Created at: 2023-08-01, 19:41:09
 *   Last updated at: 2023-08-30, 18:45:38
...
class CodeAreaCompiler {
  ...
  private convertIntoTuplesArray() {
    ...
    const arrayWithIndex = createArray.map(
      (line, originalIndex) => `${originalIndex + 1} ${line}`
    );
    ...
    const removeOtherComments = filterOnlyUseful.map(
  ...
  private addBreakAfterCommentSign(oneLine: string[]) {
    const elementIndex = oneLine.findIndex(
      el => el[0] === COMMENT_CHAR && el.length > 1
    );
    if (elementIndex !== -1) {
  ...
  private static reverseArray(
    arrayToReverse: object[],
    singleTupleIndex: number
  ): object[] {
    const reverseArray = [];
  ...
  private findIllegalSymbols(allTuplesArrayElement: string[]): boolean[] {
    const [curState, curSymb, newSymb, dir, newState, stopped] =
      allTuplesArrayElement.filter(el => el !== '' && el !== ' ');
    const currState = Boolean(ILL_STATE.find(sign => curState?.includes(sign)));
    const currSymb = Boolean(
      ILL_TAPE_SYMBOL.find(sign => curSymb?.includes(sign))
    );
    const nextSymb = Boolean(
      ILL_TAPE_SYMBOL.find(sign => newSymb?.includes(sign))
    );
    const dirSymb =
      dir !== LEG_DIR.RIGHT && dir !== LEG_DIR.LEFT && dir !== LEG_DIR.HALT;
  ...
  private tooLowTooMuchElementInTuple(
  ...
  private ignoreCommentsAfterTuple(arrayWithComment: string[]): string[] {
    const removeBlankSpaces = arrayWithComment.filter(
      el => el !== '' && el !== ' '
    );
  ...
  private static ignoreSingleLineWithComment(
    ...
    if (
      lineWithComment.indexOf(COMMENT_CHAR) === 0 &&
      lineWithComment.length > 1
    ) {
  ...
  private ifProgramHasInfiniteLoop(arrayWithTuples: string[][]) {
    ...
    for (let i = 0; i < arrayWithTuples.length; i++) {
      if (
        findMultipleLabels[i] !== '' &&
        findMultipleLabels[i] !== COMMENT_CHAR
      ) {
        const [cState, cSym, nSym, , nState] = arrayWithTuples[i].slice(0, 5);
        const ifMultipleLabelsExist = findMultipleLabels.filter(
          label => label === cState
        );
        if (!(ifMultipleLabelsExist.length > 1)) {
          const findEndlessLoop = Boolean(
            cState === nState && cSym === ANY && nSym === ANY
          );
          this.validateData(
            findEndlessLoop,
            i,
  ...
  validateCodeArea(): boolean {
    ...
    for (let i = 0; i < splitAllElements.length; i++) {
      if (splitAllElements[i][0] !== '') {
        ...
        if (splitAllElements[i][0] !== COMMENT_CHAR) {
          const ignoreComments = this.ignoreCommentsAfterTuple(
            splitAllElements[i]
          );
    ...
    this.findInitialStateLabel(
      CodeAreaCompiler.reverseArray(splitAllElements, 0)
    );
    this.ifProgramNotStopLabel(splitAllElements);
    this.ifProgramHasInfiniteLoop(splitAllElements);
    const onlyErrors = this._problemsArray.filter(
      problem => problem.danger === ERROR_LEVEL
    );
