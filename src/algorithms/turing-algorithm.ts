...
class TuringAlgorithm {
    ...
    public constructor(labels: {
        [key: string]: string
    }[], findingLabel: string, tapeValues: string[], headPosition: number) {
        this._labels = labels;
    ...
    private moveMachineHead(findingElement: { [key: string]: string }): number {
        ...
        switch (findingElement.direction) {
            case TapeValues.LEG_DIR.RIGHT:  return headPosition + 1;
            case TapeValues.LEG_DIR.LEFT:   return headPosition - 1;
            default:                        return headPosition;
        }
    };

    public fixedBlankElements(findingElement: { [key: string]: string }): {
        headPosition: number,
        tapeValues: string[]
    } {
    ...
    public settingNextElementFromTuple(findingElement: { [key: string]: string }, stepsArray: {
        [key: string]: string
    }[]) {
