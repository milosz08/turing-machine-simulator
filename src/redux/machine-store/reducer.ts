 ...
 *   Created at: 2023-07-31, 23:41:48
 *   Last updated at: 2023-08-30, 18:32:15
...
import {
  IMachineStoreReduxState,
  machineStoreReduxState,
} from '~/app-redux/machine-store/state';
import { CodeAreaModes } from '~/app-utils/code-area-modes';
import { MachineModes } from '~/app-utils/machine-modes';
import * as ActionType from './action-type';
import * as Action from './actions';

export const machineStoreReducer = createReducer(
  machineStoreReduxState,
  builder =>
    builder
      .addCase(
      ...
      .addCase(
        ...
        (state: IMachineStoreReduxState): void => {
          const { machineTuples, findingLabel, tapeValues, headPosition } =
            state;
          ...
          if (findElement) {
            const { headPosition: hP, tapeValues: tV } = ta.fixedBlankElements(
              findElement!
            );
          ...
          state.machineCustomMessage = `Machine stopped! State '${findingLabel}' has no action 
                    for symbol: '${ta.getTapeValue()}' (ln ${
                      state.actualState.prevState!.lineIndex
                    })`;
        }
      )
      .addCase(
      ...
      .addCase(
        ...
        (state: IMachineStoreReduxState): void => {
          const compiler = new CodeAreaCompiler(
            state.rawCodeAreaInput,
