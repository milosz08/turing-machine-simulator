 ...
 *   Created at: 2023-08-01, 19:57:52
 *   Last updated at: 2023-08-30, 18:43:46
...
export const CodeAreaFieldTextarea = styled.textarea<{
...
export const CodeAreaSingleRowIndicator = styled.div<{
  $prev: boolean;
  $next: boolean;
}>`
  position: absolute;
...
export const CodeAreaSingleRowHighlighter = styled.aside<{
  $prev: boolean;
  $next: boolean;
}>`
  flex-grow: 1;
  ...
  background-color: ${props => {
    if (props.$prev) {
      return NonChangeValues.DEBUG_BAR_COLOUR;
    } else if (props.$next) {
      return NonChangeValues.STATUS_BAR_COLOUR;
    }
  }};
`;
