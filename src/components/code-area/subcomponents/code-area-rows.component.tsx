...
const CodeAreaSingleRowComponent = React.lazy(() => import("../subcomponents/code-area-single-row.component"));
const CodeAreaDoubleIndicatorMarksComponent = React.lazy(() => import("../subcomponents/code-area-double-indicator-marks.component"));
...
const CodeAreaRowsComponent: React.FC = (): JSX.Element => {
    ...
    const { rawCodeAreaInput, actualState }: IMachineStoreReduxState = useSelector((state: RootState) => state.machine);
    const { codeScrollPos }: IPreferencesStoreReduxState = useSelector((state: RootState) => state.preferences);
    ...
    const generateCountingRows = Array.from({ length: countOfLines }, (_, i) => i).map(iter => (
        iter === activePrevState && iter === activeNextState ? (
            <CodeAreaDoubleIndicatorMarksComponent
                key = {iter}
                iteration = {iter}
            />
        ) : (
