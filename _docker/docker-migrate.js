...
function printSuccessMessage(spinner, message) {
    spinner.stop(true);
    console.log(`\u2713 [${currentStage++}/${allStages}] Successfully ${message}.`);
}
...
function createSpinnerAndStart(message) {
    const spinner = new Spinner(`%s [${currentStage}/${allStages}] ${message}...`);
    spinner.start();
    return spinner;
}
...
async function checkIfDockerContainerIsRunning() {
    ...
    try {
        ...
        printSuccessMessage(spinner, "checked container running state. Container is running");
    } catch (err) {
        printErrorMessage(spinner, "checked container running state. Container not running");
        throw new Error(err);
    }
}

async function createPromisifyProcess({ execCommand, messOnStart, messOnEnd, options }) {
